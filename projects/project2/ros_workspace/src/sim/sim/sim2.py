...
#    locations and tracking whether the robot has reached 
...
class Sim2_Node(rclpy.node.Node):
    def __init__(self):
        ...
        self.targets = { tuple(target) : False for target in target_list }
    ...
    def update_pose(self):
        ...
        # Cap the linear and angular velocities.
        linear = max(-2.0, min(2.0, self.twist.linear.x))
        angular = max(-2.0, min(2.0, self.twist.angular.z))
        ...
        for target in self.targets.keys():
            ...
            offset = np.array(target) - self.pose[0:2]
            squared_distance = offset[0]*offset[0] + offset[1]*offset[1]
            if squared_distance < TARGET_RADIUS+TARGET_RADIUS:
    ...
    def publish_markers(self):
        ...
        # - Targets
        target_colors = { False: std_msgs.msg.ColorRGBA(r=0.0, g=80/255, b=80/255, a=1.0),
                          True:  std_msgs.msg.ColorRGBA(r=80/255, g=80/255, b=0.0, a=1.0) }
        ...
        for i, (target, reached) in enumerate(self.targets.items()):
            ...
            target_marker.scale = geometry_msgs.msg.Vector3(x=2*TARGET_RADIUS, y=2*TARGET_RADIUS, z=0.1)
            target_marker.pose.position = geometry_msgs.msg.Point(x=target[0], y=target[1], z=0.2)
            target_marker.id = 200+i
            target_marker.type = visualization_msgs.msg.Marker.CYLINDER
            target_marker.color = target_colors[reached]
    ...
    def publish_unvisited_targets(self):
        ...
        for target, visited in self.targets.items():
            if not visited:
                cloud.points.append(geometry_msgs.msg.Point32(x=target[0], y=target[1], z=0.0))
